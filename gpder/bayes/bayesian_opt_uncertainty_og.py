...
class BayesUncertaintyOptimization():
    ...
    def minimize_uncertainty(self,
        ...
        opt_acquisition: string, default='trace'
            Quantity to optimize. Options: {'trace', 'determinant'}
        ...
        self._acq = AcquisitionFunction(kind=self.opt_acquisition)
    ...
    def _find_next_X(self, n_restarts):
        def neg_acquisition_fun(X):
            return -1 * self._acq.utility(X, Xpred=self.X_grid, gp=self.gp)
        ...
        for i in range(n_restarts):
            if self.minimizer=='fmin_l_bfgs_b':
                ...
                x, fval = self.minimizer(fun=neg_acquisition_fun,
                                         bounds=self._X_bounds_vals)
            else:
