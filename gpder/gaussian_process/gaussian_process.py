...
class GaussianProcessRegressor(MultiOutputMixin, RegressorMixin, BaseEstimator):
    ...
    based on the use of the DerAwareKernel, as described in [2] and [3].

    Parameters
    ----------
    kernel: kernel instance, default=None
        The kernel used in the GPR. DerAwareKernel is recommended.
        The parameters of the kernel (theta) are optimized during
        fitting unless the bounds are marked as "fixed".
        If None, DerAwareKernel is used with its default values.
    ...
    n_restarts_optimizer: int, default=0
        Number of times to restart the optimizer for finding the
        kernel's thetaa parameters which minimize the neg log-marginal
        likelihood. The first run of the optimizer is calculated
        on the kernel's initial theta. Subsequent runs
        (if n_restarts_optimizer > 0) are performed on parameter sets
        sampled from a log-uniform distribution randomly within the
        parameter bounds.
        If n_restarts_optimizer > 0, all parameter bounds must be
        finite.
    ...
    dy_train_: ndarray of shape (dnsamples, ndfeatures), optional
        Partial derivatives of the training target values wrt the
        features.
    ...
    def fit(self, X, y, dy=None):
        ...
        dy: ndarray of shape (nsamples, nfeatures), optional
            Partial derivatives of the training values.
        ...
        if self.has_derinfo_:
            kernel_train = self.kernel_(X=self.X_train_, dX=self.X_train_)
        ...
        try:
            ...
            raise("The kernel is not positive definite. Try increasing alpha.")
    ...
    def predict(self, X, return_cov=False, return_std=False):
        ...
        if not hasattr(self, "X_train_"):
            ...
            if self.has_derinfo_:  # TOOD: fix how we get the posterior
                kernel_post = self.kernel_(X=X_star, dX=self.X_train_, predict_X=True)
            else:
