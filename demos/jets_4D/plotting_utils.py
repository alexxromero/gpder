...
def plot_mse_uncert(bayes, X_test, y_test, save_to=None):
    ...
    ax[1].set_ylabel("Uncertainty", fontsize=16)
    for i in range(2):
        ax[i].ticklabel_format(style='sci', axis='y', scilimits=(0,0))
        ax[i].set_xlabel("Num. of Sampled Points", fontsize=16)
...
def plot_gp(y_test, y_pred, y_std, X_train, X_lower, X_upper, res,
    ...
    if dy_test is None:
        fig, ax = plt.subplots(1, 3, figsize=(14, 8.5/2),
                               squeeze=False)
        nderivs = 0
    else:
        nderivs = dy_test.shape[1]
        fig, ax = plt.subplots(1+nderivs, 3, figsize=(14, 8.5/2*(1+nderivs)),
                               squeeze=False)
...
def plot_gp_evolution(bayes, niters, X_test, save_to=None):
    nplots = niters + 1
    fig, ax = plt.subplots(nplots, 3, figsize=(8.5*0.72, 0.75*11), sharex=True, sharey=True)
    ...
    for i in range(nplots):
        ...
        X_test_utility = [bayes._acq.utility(
            X, bayes.X_opt, gp=gp) for X in X_test]
    ...
    fig.legend(handles=handles, labels=labels,
               columnspacing=1,
               ncol=3, loc=(0.16, 0.925))
    # ------------------ #
    fig.text(0.18, 0.9, r"Prediction", fontsize=12)
    fig.text(0.4, 0.9, r"Standard deviation", fontsize=12)
    fig.text(0.76, 0.9, r"Utility", fontsize=12)

    if bayes._has_derinfo:
        fig.text(0.44, 0.97, "Derivative GP", fontsize=12)
    else:
        fig.text(0.45, 0.97, "Regular GP", fontsize=12)
